...
function openUserModifyDialog() {
    ...
    if (flaged_info == 'a') {
        ...
        $("#faddr").val(row.faddr);
        $("#descb").val(row.descb);
        $("#holderName").val(row.holderName);
        $("#isHolder").val(row.fid);
    }
//	url = "<%=basePath %>/cust/updateCustumer.do?flaged=" + flaged_info;
//alert("A:"+$("#isHolder").val());
}
...
function saveUser() {
    ...
    $("#fm_cust").form("submit", {
        url : "<%=basePath %>/cust/addCustomer.do",
        onSubmit: function(data){
        	data.flaged = flaged_info;
        	//alert('TEST+'+$("#pid").val());
...
function selectHolderOk(){
    var selectedRows = $("#datagrid_holder_info").datagrid("getSelections");
    if (selectedRows.length != 1) {
        $.messager.alert("系统提示", "您没有选择任何记录！");
        return;
    }
    var hid = selectedRows[0].fid;
    var hname = selectedRows[0].holderName;
    $("#isHolder").val(hid);
...
<div class="easyui-dialog" closed="true" id="dlg_holder_info"
           style="top:10px;left:30px;width: 500px;height:400px;padding:10px 10px;" 
           buttons="#dlg-holder-buttons">
       <span style='font-size:12px' >户主列表:</span>
    <table id="datagrid_holder_info"></table>
</div>
<div id="dlg-holder-buttons">
    <a href="javascript:selectHolderOk()" class="easyui-linkbutton" iconCls="icon-ok">确定</a> 
    <a href="javascript:closeHolderDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
</div>
</div>
